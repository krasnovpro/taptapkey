...
with (panel = win.add('panel')) {
  ...
  with (add('group')) {
    var __sliderInput = add('edittext', [0, 0, 50, 25], '1 px'); ///
    var __slider = add('slider', [0, 0, 200, 15], 1, 0.1, 15); ///

    __sliderInput.active = true;
    __slider.onChanging = function (e) { ///
        var precision = (this.value < 1)? 10 : ((this.value < 5)? 4 : ((this.value < 10)? 2 : 1));
        __sliderInput.text = Math.round(this.value * precision) / precision;
    }
