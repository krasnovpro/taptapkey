...
function main() {
  var CFG = {
        ...
        isAddStroke: true, // Add stroke if not exists
...
function getRandomInRange(min, max, step) {
  ...
  var currMin = Math.min(min, max);
  var currMax = Math.max(min, max);
  var range = Math.floor((currMax - currMin) / step) + 1; 
  var random = Math.floor(Math.random() * range);
  return currMin + random * step;
}
