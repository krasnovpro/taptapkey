...
function fillStrokes(coll, swatches) {
  var hasSwatches = swatches && swatches.length > 1;
  var color;
  for (var i = 0, len = coll.length; i < len; i++) {
    color = hasSwatches ? getRandomSwatches(swatches) : generateColor();
    if (/text/i.test(coll[i].typename) && coll[i].textRange.characterAttributes.strokeWeight > 0) {
      coll[i].textRange.characterAttributes.strokeColor = color;
    } else if (coll[i].stroked) {
      coll[i].strokeColor = color;
    }
  }
}
