...
function main() {
  ...
  // alert(selPaths + ': ' + keyAngle);

  // var fpath = new File(Folder.temp + '/ai_temp.txt');
  // if (fpath.exists === true) fpath = new Folder(read(fpath));
  // if (fpath.exists !== true) fpath = Folder.temp;
  fpath = Folder.temp;

  file = fpath.fsName + "/ai_args.txt";
  fileout = fpath.fsName + "/ai_state.txt";

  var params;
  if (params = read(file))
    increment = - parseFloat(params);

  createAction(keyAngle + increment, actionSetName, actionName, fpath.fsName);
