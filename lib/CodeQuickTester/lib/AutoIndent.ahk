Array.Prototype.DefineProp('MinIndex', {Call: Array_MinIndex})
Array.Prototype.DefineProp('MaxIndex', {Call: Array_MaxIndex})

Array_MinIndex(a) {
    return (i := '', e := a.__Enum(2), [(enumerator_function)*], i)
    enumerator_function(*) {
        return e(&k, &v) && (i := k, !IsSet(v))
    }
}
...
AutoIndent(Code, Indent := "`t", Newline := "`r`n")
{
	...
	for each, Line in StrSplit(Code, "`n", "`r")
	{
		...
		First := SubStr(Text, 1, 1), Last := SubStr(Text, -1, 1)
		...
		if (First == "}")
		{
			while ((Found := SubStr(Text, (A_Index)<1 ? (A_Index)-1 : (A_Index), 1)) ~= "}|\s")
			{
				if (Found ~= "\s")
					continue
	...
	if Braces
		throw Error("Segment Open!")

	return SubStr(Out, (StrLen(Newline)+1)<1 ? (StrLen(Newline)+1)-1 : (StrLen(Newline)+1))
}
