...
function initializeAppState() {
  ...
  try {
    ...
    if (stored !== null) {
      ...
      if (!isNaN(v)) {
        document.documentElement.style.setProperty('--u', v + 'px');
        state.units = v;
      }
    } else {
      const cur = parseInt(
      ...
      state.units = cur;
      try { localStorage.setItem('units', cur); } catch (e) {}
    }
  } catch (e) {
    console.error('Error applying stored units', e);
  }
  // updateUnitDisplay();
...
function buildMenuItem(data, isRoot = false) {
  ...
  data.forEach((item, index) => {
    ...
    if (isRoot && index === 0) {
      ...
      conf.innerHTML = `
        <div class="units">
          <span onclick="setUnits(+1)" title="Scale the&#10;menu up">&plus;</span>
          <span onclick="setUnits( 0)" title="Reset menu&#10;scaling">|</span>
          <span onclick="setUnits(-1)" title="Scale the&#10;menu down">&minus;</span>
        </div>
      ...
      `;
          // <span id="unitsValue">${((appState.units ?? parseInt(getComputedStyle(document.documentElement).getPropertyValue('--u'))) || 4)}px</span>
...
// --- units helper: control CSS variable --u and persist in localStorage ---
function setUnits(delta) {
  try {
    const cur = parseInt(
      getComputedStyle(document.documentElement).getPropertyValue('--u')
    ) || appState.units || 4;
    let next = (delta == 0) ? 4 : cur + Number(delta); // zero value - reset to 4
    if (next < 2) next = 2;
    ...
    try {
      ...
      console.error('saving units', e);
    }
    // updateUnitDisplay();
  } catch (e) {
    console.error('changeUnits error', e);
  }
}

// function updateUnitDisplay() {
//   const el = document.getElementById('unitsValue');
//   if (el) {
//     const cur = parseInt(
//       getComputedStyle(document.documentElement).getPropertyValue('--u')
//     ) || appState.units || 4;
//     el.textContent = cur + 'px';
//   }
// }
